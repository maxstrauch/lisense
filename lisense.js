    ...
    for (let i = 0; i < modulesWithLicenses.length; i++) {
        ...
        if (modulesWithLicenses[i].parents) {
            ...
            if (pkgJson && pkgJson.name) {
                ...
                fields.push('N/A'); // not applicable
            }
        }
...
function compareToWhiteListFile(filename, modules) {
    var unlicensedModules = [];
    var allowedModules = []; // not licensed but declared allowed
    var content = fs.readFileSync(filename);
    const whiteList = JSON.parse(content);
    for (let i = 0; i < modules.length; i++) {
        // is this module's license not listed in white list
        var result = isLicenseOfModuleListedInWhiteList(modules[i], whiteList);
        if (result.licenseState === LicenseState.EXCEPTION) {
            allowedModules.push(result.module)
        } else if (result.licenseState === LicenseState.NONE) {
            unlicensedModules.push(result.module)
        }
    }
    for (let i = 0; i < unlicensedModules.length; i++) {
        if (i === 0) {
            console.log(`${chalk.red("ERROR:")} Found (${unlicensedModules.length}) package(s) with licenses NOT included on white list!`);
        }
        console.log(`License: ${unlicensedModules[i].license}, Module: ${unlicensedModules[i].name}`);
    }

    for (let i = 0; i < allowedModules.length; i++) {
        if (i === 0) {
            console.log(`${chalk.yellow("WARNING:")} Found (${allowedModules.length}) package(s) that were entered in the white list!`);
        }
        console.log(`License: ${allowedModules[i].license}, Module: ${allowedModules[i].name}`);
    }

    return unlicensedModules;
}

function isLicenseOfModuleListedInWhiteList(module, whiteList) {
    acceptedLicenses = [];
    moduleLicenses = [module.license];

    const regex = /^\((?<content>.*)\)$/;
    const match = module.license.match(regex);
    if (match?.groups.content) { // replace moduleLicenses in case of " OR " (||)
    ...
    if (acceptedLicenses.find(m => m.modules.length === 0)) return { module: module, licenseState: LicenseState.VALID }; // unlimited valid license found

    // test if the module is accepted
    for (let i = 0; i < acceptedLicenses.length; i++) {
        if (acceptedLicenses[i].modules.find(m => m === module.name)) {
            return { module: module, licenseState: LicenseState.EXCEPTION };
        }
    }

    return { module: module, licenseState: LicenseState.NONE }; // no valid license found
}
